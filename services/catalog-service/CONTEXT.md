...
### ActorContext
A frozen value object carrying `actor_id` (JWT `sub`), `actor_email` (JWT `email` claim — `None` for system actors like seed scripts), and `request_id` (from the `request_id` ContextVar set by `RequestResponseMiddleware`). Lives in `packages/shared` (`shared.actor.ActorContext`) and is shared across both `iam-service` and `catalog-service`. Built by the `get_actor_context` FastAPI dependency, which chains off `require_catalog_write` and calls `ActorContext.from_jwt(payload)`. Passed explicitly into every write use case as `input.actor`. Use cases dereference `input.actor.actor_id` when constructing audit events (events themselves still take `actor_id: UUID | None` for backward compatibility with existing audit payload shape).
